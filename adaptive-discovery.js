...
function render(){
 ...
 if(st.key==="synthesis")aCard.innerHTML=synth(); else aCard.innerHTML=`<p class="branchwhy">${st.why}</p>${st.q.map(fld).join("")}<div class="a-actions">${A.step?'<button class="ghost" id="prev">← Back</button>':'<span></span>'}<button class="button" id="next">${A.step===0?"Choose My Deeper Paths":"Continue"} →</button></div>`;
 bind()
}
function bind(){
